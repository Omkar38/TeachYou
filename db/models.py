...
class StoryboardRow(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True)  # storyboard_id
    job_id: Mapped[str] = mapped_column(String(36), ForeignKey("jobs.id"), index=True)
    data: Mapped[dict] = mapped_column(JSONType, default=dict)  # full Storyboard JSON
...
class StoryboardVersion(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    storyboard_id: Mapped[str] = mapped_column(String(36), ForeignKey("storyboards.id"), index=True)
    data: Mapped[dict] = mapped_column(JSONType, default=dict)  # snapshot at this point in time
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow)
