...
export type JobStatus = "PLANNING" | "PLANNED" | "QUEUED" | "RUNNING" | "FAILED" | "SUCCEEDED";
...
export async function renderJob(job_id: string): Promise<{ ok: boolean }> {
  return apiFetch<{ ok: boolean }>(`/jobs/${job_id}/render`, { method: "POST" });
}

export async function getStoryboard(job_id: string): Promise<Record<string, any>> {
  return apiFetch<Record<string, any>>(`/jobs/${job_id}/storyboard`);
}

export type StoryboardEditResponse = {
  storyboard: Record<string, any>;
  changes: Array<{ scene_index: number; field: string; value: any }>;
  explanation: string;
};

export type MediaCandidate = {
  source: "pexels" | "pixabay" | "upload" | "generated";
  url: string;
  thumb_url: string;
  duration_s: number;
  width: number;
...
export async function searchBroll(
  ...
  scene_index: number,
  query?: string,
): Promise<{ candidates: MediaCandidate[]; storyboard: Record<string, any> }> {
  return apiFetch(`/jobs/${job_id}/scenes/${scene_index}/broll`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function patchSceneBroll(
  job_id: string,
  scene_index: number,
  opts: { selected_idx?: number; style?: "whiteboard" | "stock_broll" },
): Promise<{ ok: boolean; storyboard: Record<string, any> }> {
  return apiFetch(`/jobs/${job_id}/scenes/${scene_index}/broll`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function editStoryboard(
  ...
  return apiFetch<StoryboardEditResponse>(`/jobs/${job_id}/edit`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
